        ...
        if (userAndDepart == null) {
            ...
            for (String singleBanSetName : banSetName) {
                ...
                if (redisTemplate.hasKey(singleBanSetName)) {
                    // 获取全部被ban的jwt
                    Set<Serializable> singleBanSet = redisTemplate.opsForSet().members(singleBanSetName);
                    for (Serializable singleJwt : singleBanSet) {
                        if(singleJwt.toString().equals(token)) {
                            // 若banjwt中有该token则拦截该请求
                            response.setStatusCode(HttpStatus.UNAUTHORIZED);
                            return response.writeWith(Mono.empty());
                        }
                    }
            ...
            for (Serializable str : set) {
                // 若该用户具有访问该url的权限
                if (str.toString().equals(privKey)) {
